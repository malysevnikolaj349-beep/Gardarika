...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ⚡ OPTIMIZATION: Run DB operation in a separate thread to avoid blocking the event loop.
...
async def profile(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ⚡ OPTIMIZATION: Fetch character data in a non-blocking way.
...
async def create_character_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    user_id = update.effective_user.id
    # ⚡ OPTIMIZATION: Check for existing character without blocking.
    character = await asyncio.to_thread(get_character_by_user_id, user_id)
    if character:
...
async def choose_faction(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        # Сохраняем в БД
        # ⚡ OPTIMIZATION: Perform the write operation in a separate thread.
